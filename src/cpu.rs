...
impl CPU {
    ...
    fn run(&mut self) {
        ...
        loop {
            ...
            match code {
                ...
                /* BPL */ 0x10 => self.branch(!self.status.contains(CpuFlags::NEGATIV)),

                /* BMI */ 0x30 => self.branch(self.status.contains(CpuFlags::NEGATIV)),
                ...
                /* BIT */ 0x24 | 0x2c => self.bit(&op.mode),
                ...
                /* LDX */ 0xa2 | 0xa6 | 0xb6 | 0xae | 0xbe => self.ldx(&op.mode),

                /* LDY */ 0xa0 | 0xa4 | 0xb4 | 0xac | 0xbc => self.ldy(&op.mode),
    ...
    fn update_zero_and_negative_flags(&mut self, result: u8) {
        self.status.insert(if result == 0 {
            CpuFlags::ZERO
        } else {
            CpuFlags::ZERO
        });

        self.status.insert(if result & 0b1000_0000 != 0 {
            CpuFlags::NEGATIV
        } else {
            CpuFlags::NEGATIV
        });
